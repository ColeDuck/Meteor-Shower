...
func get_title() -> String:
	return "Supernoval Expansion"
	
# Text describing the card
func get_description() -> String:
	return "Increases dust spawn rate"
	
# Bottom text showing what the upgrade actually does
func get_upgrade_text() -> String:
	return "Multiplier: " + str(get_amount(times_applied)) + " -> " + str(get_amount(times_applied + 1))
...
func do_upgrade() -> void:
	...
	StatManager.dust_spawn_mult = get_amount(times_applied)
	if times_applied >= 5:
